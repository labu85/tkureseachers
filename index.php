...
<html>
	<head>
		...
		<script>document.write('<script src="http://' + (location.host || 'localhost').slisplit(':')[0] + ':35729/livereload.js?snipver=1"></' + 'script>')</script>
			...
			if((isset($_POST['username']) && isset($_POST['password']))){
				...
				if(md5($_POST["password"]) == $pwd){
					...
					if(isset($_POST["rememberme"]) && ($_POST["rememberme"] == "true")){
						...
						setcookie("remPWD",$_POST["password"],time()+365*24*60);
					...
					if($_SESSION["Level"]==0) header("Location:member_center.php");
					if($_SESSION["Level"]==99) header("Location:admin_member.php");
				}
				if(md5($_POST["password"]) != $pwd){
					//password error
					$Err_Level = 1;
					...
					<div class="panel-body">
						...
						<form action="" method="POST" name="form">
							<div class="form-group">
								...
								<input type="text" class="form-control" id="" placeholder="" name="username" value="<?php if(isset($_COOKIE["remUSER"])) echo $_COOKIE["remUSER"];?>">
								<br/>
								<label for="">Password</label>
								<input type="password" class="form-control" id="" placeholder="" name="password" value="<?php if(isset($_COOKIE["remPWD"])) echo $_COOKIE["remPWD"];?>">
								<div class="checkbox">
									<label>
										<input type="checkbox" value="true" id="rememberme" name="rememberme" <?php if(isset($_COOKIE["isRemember"])) echo $_COOKIE["isRemember"];?>">
										Remember Me
									</label>
								</div>
